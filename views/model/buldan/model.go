...
type DB interface {
	...
	JoinGame(ID string, player Player) error
...
func (i *inMemoryDB) JoinGame(ID string, player Player) error {
    ...
    for _, p := range game.Players {
        if p.ID == player.ID {
            return fmt.Errorf("A player is already registered with this ID: %s", p.ID)
        }
    }
